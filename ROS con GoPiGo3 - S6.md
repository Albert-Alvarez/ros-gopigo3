...
- [Procedimiento](#procedimiento)
  ...
  - [Conexión a la red](#conexión-a-la-red)
    - [Antes de conectarse](#antes-de-conectarse)
    - [Conectarse](#conectarse)
  - [Encendiendo máquinas](#encendiendo-máquinas)
  - ["¡Está vivooo! ¡Vivoooooo!"](#está-vivooo-vivoooooo)
  - [Organización del espacio de trabajo](#organización-del-espacio-de-trabajo)
  - [Mover el robot con python](#mover-el-robot-con-python)
  - [Permisos en Linux](#permisos-en-linux)
  - [Ejecutar archivo python](#ejecutar-archivo-python)
  - [Otras instrucciones](#otras-instrucciones)
  - [ROS en la RPi](#ros-en-la-rpi)
  - [Inicializar el *workspace* y el *package*](#inicializar-el-workspace-y-el-package)
  - [*Packages* de terceros](#packages-de-terceros)
  - [*Package* gopigo3](#package-gopigo3)
  - [*Package* teleop_tools](#package-teleop_tools)
  - [*Package* raspicam](#package-raspicam)
  - [*Package* ydlidar](#package-ydlidar)
  - [Ver los datos en Rviz en nuestra VM](#ver-los-datos-en-rviz-en-nuestra-vm)
  - [Apagar lar RPi](#apagar-lar-rpi)
...
Para esta práctica necesitamos que la VM tenga configurada su conexión en modo *bridge*. ¿Para qué? Para que tenga su propia IP dentro de la red y la RPi pueda encontrar la VM. Es sencillo. Con la VM **apagada**, vamos a su `Configuración` y en el apartado `Red`, escogemos `Adaptador puente` en el desplegable `Conectado a`. Nada más 😁
...
<p align="center">
    <a href="assets/S6/imgs/vsc-remote-development.png">
      <img src="assets/S6/imgs/vsc-remote-development.png">
    </a>
</p>
...
Veréis que al conectarnos a la RPi nos quedaremos sin acceso a internet. Vamos a descargar antes unos paquetes que necesitaremos. En el un terminal de vuestra VM, ejecutad los siguientes comandos:
...
Esto por el momento, más adelante moveremos estos paquetes a la RPi.

### Conectarse

Para realizar las prácticas, nos conectaremos a una red WiFi generada por un router al cuál se conectan todas las RPis. **Esa red no dispone de conexión** a internet (WAN), por lo que antes de conectaros, bajaros este guión (imprimir PDF, por ejemplo) o dejaroslo abierto en vuestro navegador. La red y sus datos de conexión son los iguientes:

- SSID: RoboticsUB
- Contraseña: CorrePiCorre

## Encendiendo máquinas

¡Vamos a encender el robot! Eso sí, primeramente nos aseguramos que tengamos todo disponible: RPi, motores, LIDAR, cámara, tarjeta microSD, ... Con todo disponible y debidamente conectado, vamos a alimentar la RPi para encenderla. La RPi iniciará su OS y en unos 15~20 segundos se conectará a alguna de las redes WiFi que tiene configuradas por defecto. Algo muy importante es la dirección IP que le da el *router* a la RPi, puesto que la necesitamos para poder conectarnos a ella. En cada robot tenéis indicada su IP correspondiente.

Ahora vamos a conectarnos a la RPi desde nuestra VM. Nos vamos a VSC y hacemos clic al icono situado en la esquina inferior-izquierda ![](assets/S6/imgs/icono-remote.png) En la parte superior de la aplicación, se nos abrirá un desplegable. Allí seleccionamos `Remote-SSH: Connect to Host...`. Hecho esto sale una lista con todos los sistemas remotos que tenemos configurados. Obvio. No hemos configurado todavía ninguno y por eso está la lista vacía (cuando tengamos configurado nuestro sistema remoto, solamente deberemos de seleccionarlo en la lista). Clicamos a `+ Add New SSH Host...`.  Seguidamente, nos pide ingresar la instrucción de conexión. Es esta:

```bash
ssh pi@XXX.XXX.XXX.XXX
```

donde `XXX.XXX.XXX.XXX` es la IP de vuestra RPi. Una vez ingresada la instrucción, pulsamos <kbd>ENTER</kbd>. En el siguiente desplegable, lo mismo, <kbd>ENTER</kbd>. Ya está todo configurado, ahora vamos a la ventana *pop-up* que nos ha aparecido en la esquina inferior-derecha de la aplicación y le damos a `Connect`.

<p align="center">
    <a href="assets/S6/imgs/pop-up-connect.png">
      <img src="assets/S6/imgs/pop-up-connect.png">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/terminal-remote.png">
      <img src="assets/S6/imgs/terminal-remote.png">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/vsc-final.png">
      <img src="assets/S6/imgs/vsc-final.png">
    </a>
</p>

> Puesto que hemos escogido una carpeta con un nivel jerarquico muy elevado (casi el "equivalente" a la unidad `C:\` en Windows), el número de archivos contenidos en esta carpeta es tan elevado que VSC no actualiza/refresca el explorador de archivos automáticamente. Si creamos un archivo desde el terminal, este nos aparecerá en el Explorador de Archivos después de refrescarlo manualmente mediante el correspondiente botón.
...
Ahora emularemos al <a href="https://es.wikipedia.org/wiki/V%C3%ADctor_Frankenstein">Dr. Víctor Frankenstein</a> y daremos vida a nuestro robot haciendo que se mueva. Como Adam, que es como se llamaba realmente la criatura creada por el Dr. Frankenstein (os dejo ese detalle ahí para cuando vayáis a Pasapalabra 🧟‍♂️ (me debéis un porcentaje del bote)), <del>nuestro robot necesita alimentación para funcionar. Así que lo primero que haremos será conectar nuestro pack de pilas al conector jack que hay en la placa de circuito impreso (PCB) roja. De este modo, estaremos alimentando los motores con los 12 V que necesitan. El transformador de la RPi solo da 5 V. Se pueden dejar conectados ambos y es lo más recomendable mientras que está operando con la RPi sin que el robot deba moverse. Así, la RPi se alimenta del transformador y se alarga la carga de las baterías. Cuando tengamos que hacer que el robot se mueva, desconectamos el conector microUSB de la RPi y, al acabar la prueba y volver a coger nuestro robot para programar, volvemos a conectar el microUSB. Fácil.</del>

> Ante el robot utilizaba pilas. Ahora el robot funciona con un *power banck* que alimenta directamente tanto la RPi como los motores.
...
<p align="center">
    <a href="assets/S6/imgs/pwd.png">
      <img src="assets/S6/imgs/pwd.png">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/prompt-path.png">
      <img src="assets/S6/imgs/prompt-path.png">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/move_motors.png">
      <img src="assets/S6/imgs/move_motors.png">
    </a>
</p>

> Si no os aparecen las carpetas creadas en el explorador de archivos de VSC, dadle al icono de refrescar situado justo encima ![](assets/S6/imgs/refresh.png)
...
> **IMPORTANTE:** Puede ser tentador copiar/pegar el código. No se puede comprobar (o sí... 😈) si lo habéis hecho o no, ni tampoco se puede obligar. Pero es altamente recomendable que siempre intentéis escribir todo aquello que programéis. Es una recomendación para vuestro propio interés 😉
...
<p align="center">
    <a href="assets/S6/imgs/ls.png">
      <img src="assets/S6/imgs/ls.png">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/ls-l.png">
      <img src="assets/S6/imgs/ls-l.png">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/Bear-Grylls.jpg">
      <img src="assets/S6/imgs/Bear-Grylls.jpg">
    </a>
</p>
...
Guardamos el archivo y, igual que antes, le damos permisos de ejecución al usuario propietario (nosotros). La estructura de nuestro *workspace* sería
...
> Cuidado con confundir nuestra carpeta `workspace` con el término *workspace* en ROS, que para ROS viene a significar "proyecto". Siempre indicaré la carpeta  `wokspace` en formato código, mientras que el *workspace* de ROS lo indico como texto en cursiva.
...
Finalmente, vamos a ejecutar nuestro nodo `move_motors.py` de nuestro *package* `hello-gopigo3`. Para ello, sabemos que primeramente debemos de ejecutar `roscore`. Abrimos un segundo terminal en VSC. Lo hacemos clicando en el siguiente icono ![](assets/S6/imgs/icono-nueva-terminal.png) encima del terminal. Allí ejecutamos el comando `roscore`.

<p align="center">
    <a href="assets/S6/imgs/roscore.png">
      <img src="assets/S6/imgs/roscore.png">
    </a>
</p>

Volvemos al primer terminal yendo al desplegable al lado del icono ![](assets/S6/imgs/icono-nueva-terminal.png). Allí, **una vez asegurado que el robot no se irá al precipicio**, ejecutamos
...
Hemos cogido (*copy*/*paste*) un paquete que os hemos guardado préviamente en la carpeta `~/ros_packages/`. Después de este comando, la estructura de nuestro *workspace* sería la siguiente
...
<p align="center">
    <a href="assets/S6/imgs/rosgraph.svg">
      <img src="assets/S6/imgs/rosgraph.svg">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/roslaunch-gopigo3.png">
      <img src="assets/S6/imgs/roslaunch-gopigo3.png">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/rostopic-list.png">
      <img src="assets/S6/imgs/rostopic-list.png">
    </a>
</p>
...
Este paquete nos lo hemos descargado en nuestra VM antes de conectarnos a la RPi. Simplemente cogemos la carpeta en `~/ros_packages/teleop_tools` en nuestra VM y la arrastramos, entera, a la carpeta `~/workspace/ros/session1/src` de nuestra RPi (la arrastráis hasta ahí mediante el Explorador de Archivos de VSC).
...
Con `/key_vel:=/cmd_vel` lo que hacemos es [remapear los argumentos](http://wiki.ros.org/Remapping%20Arguments) del nodo para que publique en el *topic* `/cmd_vel` (que es al que está suscrito el nodo gopigo3) en lugar del *topic* original `/key_vel`. Seguidamente, pulsamos la flecha <kbd>↑</kbd> de nuestro teclado y ... ¡se mueveee!
...
Este paquete nos lo hemos descargado en nuestra VM antes de conectarnos a la RPi. Simplemente cogemos la carpeta en `~/ros_packages/raspicam_node` en nuestra VM y la arrastramos, entera, a la carpeta `~/workspace/ros/session1/src` de nuestra RPi (la arrastráis hasta ahí mediante el Explorador de Archivos de VSC).

Una vez añadido, nos vamos al directorio raíz de nuestro *workspace* y hacemos un `catkin_make`.
...
<p align="center">
    <a href="assets/S6/imgs/error-catkin-make.png">
      <img src="assets/S6/imgs/error-catkin-make.png">
    </a>
</p>
...
Cogemos la carpeta en `~/ros_packages/image_transport_plugins` en nuestra VM y la arrastramos, entera, a la carpeta `~/workspace/ros/session1/src` de nuestra RPi (la arrastráis hasta ahí mediante el Explorador de Archivos de VSC).

Si hiciésemos otro `catkin_make`, se nos diría que faltan otros *packages*. Los *packages* en cuestión son el cv_bridge y el image_transport. Los cogemos de la carpeta `~/ros_packages/`
...
El último *package* que instalaremos en esta sesión. Este *package* se encarga de operar el LIDAR y publicar sus medidas en un *topic*. Este si que lo tenemos disponible en la RPi, así que, para instalarlo, ejecutamos el siguiente comando en la carpeta `src` de nuestro *workspace*
...
<p align="center">
    <a href="assets/S6/imgs/lidar.png">
      <img src="assets/S6/imgs/lidar.png">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/datos-lidar.png">
      <img src="assets/S6/imgs/datos-lidar.png">
    </a>
</p>
...
<p align="center">
    <a href="assets/S6/imgs/rviz.png">
      <img src="assets/S6/imgs/rviz.png">
    </a>
</p>
...
¡El último paso de esta sesión y ya estamos! Igual de importante que el resto de los pasos anteriores es apagar la RPi de manera correcta. Para ello, en un terminal conectado a la RPi (es decir, desde VSC) ejecutamos el comando
